...
const QA_NOTES = {
  ...
  'TC_SAUD_008': `Bug confirmed but severity revised after evidence screenshot capture. The /v1/ecgs endpoint at the Railway Flutter app URL returns HTTP 200 with the Flutter app\'s index.html (SPA fallback routing) rather than actual ECG JSON records. The Railway deployment serves all unrecognised paths — including /v1/* API paths — back to the SPA router, which returns index.html with a 200 status. The actual Go backend API likely lives on a separate Railway service at a different URL. The test correctly identified that the URL returns 200 instead of 401/403, but no PHI is directly exposed at this particular URL. The risk is a server misconfiguration that could become a real PHI exposure if the backend is ever co-deployed on the same domain without proper auth middleware on /v1/* routes. Screenshot shows the actual HTTP 200 response with HTML body clearly. Recommended action: (1) Confirm the Go backend service returns 401/403 on unauthenticated /v1/ecgs calls at its actual hostname. (2) Configure Railway proxy to return 401 for /v1/* paths that lack a valid Authorization header.`,
};
...
const BUG_INFO = {
  ...
  // ── New module bugs ───────────────────────────────────────────────────────
  'TC_SECH_007': { shot: 'SECH_007_brute_force.png', desc: 'No account lockout after 7 consecutive wrong password attempts. All 7 attempts returned the same login error with no throttling, no CAPTCHA, and no lockout message. WHY THIS IS HIGH SEVERITY: An attacker with a password list can attempt credentials indefinitely — there is no mechanism to slow or stop automated guessing. A doctor\'s account controlling patient PHI is exposed to credential stuffing attacks. HIPAA Security Rule §164.312(d) requires reasonable access controls. Recommended fix: lock account for 15 minutes after 5 failed attempts; add exponential backoff; send an alert email to the account owner when repeated failures are detected.' },
  'TC_SAUD_008': { shot: 'SAUD_008_unauth_api.png', desc: 'GET /v1/ecgs returns HTTP 200 when called without an Authorization header after logout. The response body is the Flutter app\'s HTML (SPA fallback) rather than JSON ECG records — meaning no raw PHI is directly returned at this URL. HOWEVER this is still a server misconfiguration: the Railway deployment routes all unrecognised paths (including API paths) to the SPA\'s index.html, which means the API endpoint returns 200 instead of the expected 401/403. This masks whether the actual Go backend API correctly rejects unauthenticated requests. RISK: If the backend API is later moved to the same domain without proper auth middleware, this path would silently start returning real PHI. Recommended action: (1) Confirm the Go backend returns 401/403 for unauthenticated /v1/ecgs calls at its actual service URL. (2) Add explicit 401 responses for /v1/* routes in the Railway proxy config so API paths never fall through to the SPA router.' },
};
